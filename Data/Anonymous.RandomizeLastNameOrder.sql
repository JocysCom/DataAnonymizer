...
IF OBJECT_ID('[Anonymous].[RandomizeLastNameOrder]', 'P') IS NOT NULL 
  DROP PROCEDURE [Anonymous].[RandomizeLastNameOrder]
GO

CREATE PROCEDURE [Anonymous].[RandomizeLastNameOrder] 
AS

-- EXEC [Anonymous].[RandomizeLastNameOrder]
-- SELECT TOP (10000) * FROM [Anonymous].[LastName]
...
SELECT @total = COUNT(*)
FROM [Anonymous].[LastName] p WITH(NOLOCK)
